...
public class DateValue extends GDateValue implements Comparable {
    ...
    public CharSequence getPrimitiveStringValue() {
        ...
        int yr = year;
        if (year <= 0) {
        	// no year zero in lexical space for XSD 1.0
            yr = -yr + (xsd10rules ? 1 : 0);           
            if (yr != 0) {
                sb.append('-');
            }
        }
		appendString(sb, yr, yr > 9999 ? Integer.toString(yr).length() : 4);
        sb.append('-');
    ...
    public CalendarValue add(DurationValue duration) throws XPathException {
        if (duration instanceof DayTimeDurationValue) {
			...
			XPathException err = new XPathException("Date arithmetic is not supported on xs:duration, only on its" +
				" subtypes");
    ...
    public int compareTo(Object v2) {
        try {
        	...
        	LOGGER.error("DateTime comparison requires access to implicit timezone", err);
	...
	public static int getJulianDayNumber(int year, int month, int day) {
		...
		if (z >= 0) {
			...
			// number of leap years in 12000 years
			return j - (365 * 12000 + 12000 / 4 - 12000 / 100 + 12000 / 400);
		}
	}
    ...
    public static DateValue dateFromJulianDayNumber(int julianDayNumber) {
        if (julianDayNumber >= 0) {
        	// +1 adjustment for days starting at noon
			...
			DateValue dt = dateFromJulianDayNumber(
				julianDayNumber + (365 * 12000 + 12000 / 4 - 12000 / 100 + 12000 / 400));
            dt.year -= 12000;
            return dt;
        }
    }
	...
	public static int getDayOfWeek(int year, int month, int day) {
		...
		// 1800-01-05 - any Monday would do
		d -= 2378500;
		while (d <= 0) {
			// any sufficiently high multiple of 7 would do
			d += 70000000;
		}
		return (d - 1) % 7 + 1;
	}
