...
public abstract class HTMLEmitter extends XMLEmitter {
	...
	private static final Logger LOGGER = Logger.getLogger(HTMLEmitter.class);
    ...
    */
    ...
    private static boolean isBooleanAttribute(String element, String attribute, String value) {
		return attribute.equalsIgnoreCase(value) && booleanAttributes.contains(attribute) &&
    ...
    protected void openDocument() throws XPathException {
		...
		if ("yes".equals(byteOrderMark) &&
			...
			try {
				...
				LOGGER.error("Could not write content into the document", err);
			}
		}
        inScript = -1000000;
    }
    ...
    */
	...
	public void startElement(NodeName elemName, SchemaType typeCode, int locationId, int properties)
		...
		if (elemName.isInNamespace("") &&
			(parentElement.equalsIgnoreCase("script") || parentElement.equalsIgnoreCase("style"))) {
			inScript = 0;
		}
        inScript++;
    ...
    */
    @Override
	protected void writeAttribute(NodeName elCode, String attname, CharSequence value, int properties)
		throws XPathException {
		try {
			if (uri.length() == 0 && isBooleanAttribute(parentElement, attname, value.toString())) {
    ...
    */
	...
	public void processingInstruction(String target, CharSequence data, int locationId, int properties)
		...
		for (int i = 0; i < data.length(); i++) {
			if (data.charAt(i) == '>') {
				XPathException err = new XPathException("A processing instruction in HTML must not contain a > " +
					"character");
