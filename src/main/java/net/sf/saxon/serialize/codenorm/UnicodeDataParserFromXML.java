...
class UnicodeDataParserFromXML {
    ...
    static NormalizerData build(Configuration config) throws XPathException {
		...
		if (canonicalClassKeys != null) {
			String canonicalClassKeysValue = canonicalClassKeys.getStringValue();
			readCanonicalClassTable(canonicalClassKeysValue, canonicalClassKeysValue, canonicalClass);
		}
		...
		if (decompositionKeys != null) {
			String decompositionKeysValue = decompositionKeys.getStringValue();
			readDecompositionTable(decompositionKeysValue, decompositionKeysValue, decompose, compose, isExcluded,
				isCompatibility);
		}
