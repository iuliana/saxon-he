...
public class SortKeyDefinition implements Serializable {
	...
	public void typeCheck(ExpressionVisitor visitor, ExpressionVisitor.ContextItemType contextItemType) throws XPathException {
		...
		if (language instanceof StringLiteral && ((StringLiteral)language).getStringValue().length() != 0) {
			String langAttribute = ((StringLiteral) language).getStringValue();
			if (!langAttribute.isEmpty()) {
				ValidationFailure vf = StringConverter.STRING_TO_LANGUAGE.validate(langAttribute);
				if (vf != null) {
					throw new XPathException("The lang attribute of xsl:sort must be a valid language code",
						"XTDE0030");
				}
			}
		}
