...
public class DOM4JObjectModel extends TreeModel implements ExternalObjectModel, Serializable {
     ...
     * @throws net.sf.saxon.trans.XPathException if the object is recognized as a DOM4J node but cannot be
     * wrapped
     */
    ...
    private Sequence convertObjectToXPathValue(Node object, Configuration config) throws XPathException {
        if (isRecognizedNode(object)) {
            if (object instanceof Document) {
                return wrapDocument(object, "", config);
            } else {
                Document root = getDocumentRoot(object);
                DocumentInfo docInfo = wrapDocument(root, "", config);
    ...
    public NodeInfo wrapNode(DocumentInfo document, Node node) {
