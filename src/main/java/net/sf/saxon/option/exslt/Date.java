...
public final class Date {
	...
	private static final String[] MONTHS_ABBREVIATED = {"Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep",
		"Oct", "Nov", "Dec"};
	
	private static final String[] DAYS = {"Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"};
	
	private static final String[] DAYS_ABBREVIATED = {"Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"};
    ...
    public static String date(XPathContext context, String datetimeIn) {
		...
		if (datetimeIn.indexOf('T') >= 0) {
			...
			return cr instanceof ValidationFailure ? "" : ((DateTimeValue) cr).toDateValue().getStringValue();
		} else {
			ConversionResult cr = DateValue.makeDateValue(datetimeIn, rules);
			return cr instanceof ValidationFailure ? "" : ((AtomicValue) cr).getStringValue();
		}
	}
	...
	public static String time(XPathContext context, String dateTime) {
		...
		if (dateTime.indexOf('T') >= 0) {
			...
			return cr instanceof ValidationFailure ? "" : ((DateTimeValue) cr).toTimeValue().getStringValue();
		} else {
    ...
    public static double year(XPathContext context, String datetimeIn) {
		...
		try {
			...
			if (cr instanceof GMonthValue || cr instanceof GMonthDayValue || cr instanceof GDayValue ||
				cr instanceof TimeValue) {
				return Double.NaN;
			}
			AtomicValue year = ((CalendarValue) cr).getComponent(Component.YEAR);
			return ((NumericValue) year).getDoubleValue();
        } catch (XPathException e) {
        	LOGGER.error("Could not obtain the year of the date as a number", e);
            return Double.NaN;
        }
    }
    ...
    public static String monthAbbreviation(XPathContext context, String date) {
        ...
        return MONTHS_ABBREVIATED[(int)m - 1];
    }
    ...
    public static double dayInMonth(XPathContext context, String dateTime) throws XPathException {
        ...
        try {
			...
			if (cr instanceof GYearValue || cr instanceof GYearMonthValue || cr instanceof GMonthValue ||
				cr instanceof TimeValue) {
				return Double.NaN;
			}
        	...
        	LOGGER.error("Could not obtain the day number in the specified date: " + dateTime, e);
            return Double.NaN;
        }
    }
    ...
    public static String dayAbbreviation(XPathContext context, String dateTime) throws XPathException {
		...
		return DAYS_ABBREVIATED[(int) d - 1];
    }
    ...
    public static double hourInDay(XPathContext context, String dateTime) {
        ...
        try {
        	...
        	LOGGER.error("Could not obtain the hour of the day in the specified date: " + dateTime, e);
            return Double.NaN;
        }
    }
    ...
    public static double minuteInHour(XPathContext context, String dateTime) {
        ...
        try {
        	...
        	LOGGER.error("Could not obtain the minute of the hour in the specified date: " + dateTime, e);
            return Double.NaN;
        }
    }
    ...
    public static double secondInMinute(XPathContext context, String dateTime) throws XPathException {
        ...
        try {
        	...
        	LOGGER.error("Could not obtain the second of the minute in the specified date: " + dateTime, e);
            return Double.NaN;
        }
    }
	...
	public static String add(XPathContext context, String datetimeIn, String durationIn) throws XPathException {
		...
		YearMonthDurationValue v1m = (YearMonthDurationValue) Converter.DURATION_TO_YEAR_MONTH_DURATION.convert(v1);
		// v1.convertPrimitive(BuiltInAtomicType.YEAR_MONTH_DURATION, false, rules);
		DayTimeDurationValue v1s = (DayTimeDurationValue) Converter.DURATION_TO_DAY_TIME_DURATION.convert(v1);
		// v1.convertPrimitive(BuiltInAtomicType.DAY_TIME_DURATION, false, rules);
		DateTimeValue sum = (DateTimeValue) v0.add(v1m).add(v1s);
		return Converter.convert(sum, cv0.getPrimitiveType(), context.getConfiguration().getConversionRules())
			.getStringValue();
	}
	...
	private static DurationValue addDurationValues(DurationValue dv0, DurationValue dv1, XPathContext context) {
		YearMonthDurationValue dv0m = (YearMonthDurationValue) Converter.DURATION_TO_YEAR_MONTH_DURATION
			.convert((DurationValue) dv0);
		// ((DurationValue)dv0).convertPrimitive(BuiltInAtomicType.YEAR_MONTH_DURATION, false, rules);
		DayTimeDurationValue dv0s = (DayTimeDurationValue) Converter.DURATION_TO_DAY_TIME_DURATION
			.convert((DurationValue) dv0);
		// ((DurationValue)dv0).convertPrimitive(BuiltInAtomicType.DAY_TIME_DURATION, false, rules);
		YearMonthDurationValue dv1m = (YearMonthDurationValue) Converter.DURATION_TO_YEAR_MONTH_DURATION
			.convert((DurationValue) dv1);
		// ((DurationValue)dv1).convertPrimitive(BuiltInAtomicType.YEAR_MONTH_DURATION, false, rules);
		DayTimeDurationValue dv1s = (DayTimeDurationValue) Converter.DURATION_TO_DAY_TIME_DURATION
			.convert((DurationValue) dv1);
		...
		return new DurationValue(positive, 0, months, 0, 0, 0, (int) (micros / 1000000), (int) (micros % 1000000),
    ...
    public static String duration(double seconds) {
        DayTimeDurationValue v = DayTimeDurationValue.fromSeconds(BigDecimal.valueOf(seconds));
    ...
    public static double seconds(XPathContext context, String datetimeIn) throws XPathException {
		...
		if (cr instanceof DateTimeValue || cr instanceof DateValue || cr instanceof GYearValue ||
			cr instanceof GYearMonthValue) {
        ...
        if (cr instanceof DurationValue) {
            ...
            return duration.getYears() != 0 || duration.getMonths() != 0 ? Double.NaN : duration.getLengthInSeconds();
        } else {
            return Double.NaN;
        }
    }
