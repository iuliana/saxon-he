...
implements Locator, Container, InstructionInfo {
	...
	public SequenceType makeSequenceType(String sequenceType) throws XPathException {
		getStaticContext();
		try {
			DecimalValue xPathLanguageLevel = staticContext.getXPathLanguageLevel();
			ExpressionParser parser = getConfiguration().newExpressionParser("XP", false, xPathLanguageLevel);
			parser.setLanguage(ExpressionParser.XPATH, xPathLanguageLevel);
